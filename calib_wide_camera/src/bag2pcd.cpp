...
string in_datapath = "/media/sam/data/IJRR/extrinsic/gnss/";
...
void avia_callback(const livox_ros_driver::CustomMsg::ConstPtr& msg)
{
  ...
  pc_ = mypcl::append_cloud(pc_, *_pc);
  msg_cnt++;
  ROS_INFO_STREAM("pc size "<<pc_->points.size()<<" "<<msg_cnt);
}
...
int main(int argc, char** argv)
{
  ...
  while(ros::ok())
  {
    ...
    if(msg_cnt == 94)
    {
      downsample_voxel(*pc_, 0.01);
      pcl::io::savePCDFileBinary(in_datapath + "4.pcd", *pc_);
      break;
    }
  }
  return 0;
